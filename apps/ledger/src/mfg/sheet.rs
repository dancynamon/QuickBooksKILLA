...
pub fn standard_cost(
    ...
    for line in &sheet.lines {
        let component_id = line
            .component_id()
            .ok_or_else(|| MfgError::LineNamesNothing {
...
pub fn sensitivity(
    ...
    let apply_material_factor = |factor: Decimal| -> Result<Money, MfgError> {
        let adjusted = round_money(
            to_decimal(base.material) * factor,
            RoundingPolicy::LineExtension,
        )?;
        Ok(base
            .total
            .checked_sub(base.material)?
