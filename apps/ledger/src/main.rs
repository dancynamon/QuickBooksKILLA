...
fn parse_mfg_adjust(mut args: Args<'_>) -> Result<MfgCommand, UsageError> {
    ...
    while let Some(flag) = args.next() {
        match flag.as_str() {
            ...
            "--amount" => amount = Some(parse_money(&next_value(&mut args, "--amount")?, "--amount")?),
...
fn run_mfg(command: MfgCommand) -> Result<ExitCode, CommandError> {
    match command {
        ...
        MfgCommand::SheetAdd { db, company, file } => run_mfg_sheet_add(&db, &company, &file).map(ok),
...
fn run_mfg_receive(
    ...
    let ledger = Ledger::open(db).map_err(runtime)?;
    let result = landed::receive_materials(&ledger, company, receipt, Utc::now()).map_err(runtime)?;
    println!(
...
fn run_mfg_sheet_cost(
    ...
    for line in &sheet.lines {
        if let Some(id) = line.component_id() {
            if !materials.contains_key(id) {
                materials.insert(id.to_string(), ledger.material(company, id).map_err(runtime)?);
            }
        }
    }
    let cost = mfg_sheet::standard_cost(&sheet, &materials).map_err(runtime)?;
    println!("STANDARD COST   {sheet_id}\n");
    println!("material   {:>10}   (gross {} bf)", fmt_money(cost.material), cost.material_gross_bf);
    ...
    if let Some(price) = price {
        ...
        println!("\nat price {}: margin {}\n", fmt_money(price), fmt_pct(margin));
        println!("SENSITIVITY\n");
...
fn run_mfg_build(db: &std::path::Path, company: &str, file: &std::path::Path) -> Result<(), CommandError> {
...
fn run_mfg_variance(
    ...
    for row in &rows {
        println!(
            ...
            row.variance_pct.map(fmt_pct).unwrap_or_else(|| "-".to_string()),
        );
    }
    Ok(())
}
...
fn run_mfg_onhand(db: &std::path::Path, company: &str) -> Result<(), CommandError> {
    ...
    println!("{:<20}{:>12}{:>14}{:>14}", "material", "qty", "avg cost", "value");
